...
function renderBrief(){const season=seasonShows();const profiles=season.shows.map(showProfile).sort((a,b)=>b.score-a.score);const med=SCORE_MED;const high=profiles.filter(p=>p.score>=med).slice(0,3);const watch=profiles.filter(p=>p.score<med&&p.metrics.count>0).slice(0,3);const withData=profiles.filter(p=>p.metrics.count>0);const avgGG=avg(withData.map(p=>p.metrics.gg));const avgIndex=avg(withData.map(p=>p.metrics.index));let status='Needs More Data', klass='neutral', msg='Not enough matched tour records yet to assess revenue performance for this season.';if(avgGG!=null){status=avgGG>=80?'Strong Revenue Season':avgGG>=60?'Revenue In Range':'Revenue Needs Review';klass=avgGG>=80?'good':avgGG>=60?'neutral':'warn';const capNote=avgIndex!=null?` Bushnell is indexing at ${num(avgIndex,0)} vs national capacity where Hartford records exist.`:'';msg=`Season shows averaging ${pct(avgGG,0)} GG% of gross potential — ${avgGG>=80?'demand is converting to revenue well.':avgGG>=60?'revenue is tracking within a normal range.':'revenue efficiency is below expectations; review pricing and demand signals.'}${capNote}`;}
