...
function renderAll(){const dates=ALL.map(d=>d.week_of).filter(Boolean).sort();$('dataStatus').innerHTML=`<strong>${ALL.length.toLocaleString()}</strong> records · <strong>${new Set(dates).size.toLocaleString()}</strong> weeks total`;const _fsEl=$('filteredStatus');if(_fsEl){const _sRows=ALL.filter(d=>fiscalYear(d.week_of)===STATE.season);const _last=dates[dates.length-1]||null;_fsEl.innerHTML=`Filtered: <strong>${_sRows.length.toLocaleString()}</strong> records · Last report: <strong>${_last?fmtWeek(_last):'—'}</strong>`;}
  const profiles=seasonShows().shows.map(showProfile);const active=profiles.filter(p=>p.metrics.count>0);SCORE_MED=median(profiles.map(p=>p.score));$('kSeason').textContent=STATE.season.slice(2).replace('-20','-');$('kScheduled').textContent=profiles.length;$('kTourCap').textContent=pct(avg(active.map(p=>p.metrics.cap)),0);$('kPeerCap').textContent=pct(avg(active.map(p=>p.metrics.peerCap)),0);$('kIndex').textContent=num(avg(active.map(p=>p.metrics.index)),0);$('kHighFit').textContent=profiles.filter(p=>p.score>=SCORE_MED).length;$('kWatch').textContent=profiles.filter(p=>p.score<SCORE_MED&&p.metrics.count>0).length;renderActive();}
...
function renderBrief(){const season=seasonShows();const profiles=season.shows.map(showProfile).sort((a,b)=>b.score-a.score);const med=SCORE_MED;const high=profiles.filter(p=>p.score>=med).slice(0,3);const watch=profiles.filter(p=>p.score<med&&p.metrics.count>0).slice(0,3);const withData=profiles.filter(p=>p.metrics.count>0);const avgIndex=avg(withData.map(p=>p.metrics.index));let status='Needs More Hartford Data', klass='neutral', msg='Most of this view is using national and peer tour behavior until Hartford/Bushnell-specific records are available.';if(avgIndex){status=avgIndex>=105?'Above National Fit':avgIndex>=95?'In Line With National':'Below National Fit';klass=avgIndex>=105?'good':avgIndex>=95?'neutral':'warn';msg=`Selected season shows are indexing at ${num(avgIndex,0)} against national capacity where Hartford records exist.`}
  $('tab-brief').innerHTML=`<div class="section-divider"><h2>Executive Brief</h2><div class="section-divider-line"></div><div class="section-divider-meta">${season.name} · programming decision view</div></div><div class="brief"><div class="callout ${klass}"><h3>${status}</h3><p>${msg}</p></div><div class="card"><div class="card-hd">What this page answers</div><div class="card-sub">How current shows are doing, how previous shows performed, and what next season candidates may do for Bushnell.</div><div class="metric-row"><div class="metric"><div class="val">${pct(avg(withData.map(p=>p.metrics.cap)),0)}</div><div class="lbl">Tour Capacity</div></div><div class="metric"><div class="val">${pct(avg(withData.map(p=>p.metrics.peerCap)),0)}</div><div class="lbl">Peer Capacity</div></div><div class="metric"><div class="val">${num(avg(withData.map(p=>p.score)),0)}</div><div class="lbl">Avg Fit</div></div></div></div></div><div class="grid grid-3" style="margin-top:18px"><div class="card"><div class="card-hd">Strongest Signals</div><div class="rank-list">${rankItems(high,p=>p.show.title, p=>`Fit ${p.score} · ${pct(p.metrics.cap,0)} tour cap`, p=>p.score)}</div></div><div class="card"><div class="card-hd">Watchlist</div><div class="rank-list">${watch.length?rankItems(watch,p=>p.show.title,p=>`Fit ${p.score} · ${p.metrics.count} records`,p=>p.score):'<div class="empty">No current watchlist items with available data.</div>'}</div></div><div class="card"><div class="card-hd">Programming Signals</div><div class="insight-list" id="briefInsights"></div></div></div><div class="grid grid-2" style="margin-top:18px"><div class="card"><div class="card-hd">Season Show Fit</div><canvas id="cBriefFit"></canvas></div><div class="card"><div class="card-hd">Capacity: Tour vs Peer</div><canvas id="cBriefCap"></canvas></div><div class="card full" style="border-left:3px solid var(--rule)"><div class="card-hd" style="margin-bottom:10px">How to Use This View</div><div class="grid grid-2"><div class="note"><div class="note-hd">What the Planning Signal measures</div><div class="note-body">Four separate signals: <strong>Demand</strong> (attendance), <strong>Revenue</strong> (monetization efficiency), <strong>Peer</strong> (Bushnell-size venue behavior), and <strong>Confidence</strong> (evidence depth). Scores are calibrated against the <strong>season median</strong> — above median means stronger relative to this season's actual touring market.</div></div><div class="note"><div class="note-hd">What it does not include</div><div class="note-body">Deal terms, guarantees &amp; expense structure &nbsp;·&nbsp; Local Bushnell audience history &nbsp;·&nbsp; Routing and availability &nbsp;·&nbsp; Pre-sale pace &nbsp;·&nbsp; Group or donor strategy &nbsp;·&nbsp; Competing local events &nbsp;·&nbsp; Artistic or mission priority. <strong>Revenue Signal is not Net Profit.</strong> Use the Planning Signal to start the conversation, not end it.</div></div></div></div></div>`;
...
function loadShowIntel(shows) {
  ...
  tryFetch(SHOWS_JSON_URLS).then(data => {
    ...
    const map = {};
    data.forEach(s => { map[normalizeName(s.show)] = s; });
