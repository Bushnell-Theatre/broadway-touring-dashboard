...
def query_wikidata(show_name):
    ...
    Returns dict with opening_date, closing_date, tony_wins, tony_nominations,
    composer, lyricist, and wikipedia_url — or None if not found.
    """
    sparql = _sparql_client()
    escaped = _escape_sparql_string(show_name)

    query = f"""
    SELECT DISTINCT ?item ?openDate ?closeDate ?composerLabel ?lyricistLabel ?article
    WHERE {{
      VALUES ?label {{ "{escaped}"@en }}
      ?item rdfs:label ?label .
      ?item wdt:P31/wdt:P279* wd:Q2743 .         # instance of musical
      OPTIONAL {{ ?item wdt:P571 ?openDate . }}   # inception / opening date
      OPTIONAL {{ ?item wdt:P576 ?closeDate . }}  # dissolved / closing date
    ...
    try:
        ...
        print(f"  Wikidata error for '{show_name}': {e}")
        return None
...
# ── MAIN ──────────────────────────────────────────────────────────────────────
def main():
    # Allow optional --season argument: python scrape_shows.py --season 2024-2025
    ...
    # Load existing shows.json if present — preserve manual overrides
    existing = {}
    if DATA_OUT.exists():
    ...
    for i, show in enumerate(shows, 1):
        ...
        # Reuse existing record but still refresh if older than 30 days
        if show in existing:
            rec = existing[show]
