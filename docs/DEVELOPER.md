...
├── box_office.html         Box office scenario model — suspended, not an
...
│   │
│   └── pages/              NOT currently loaded by any page — see note below
│       ├── programming.page.js
...
└── data/
    ...
    ├── shows.json                   Show metadata. Enrichment suspended — committed
    │                                to the repo as retained from before suspension,
    │                                not regenerated by the current pipeline
    ├── seasons.json                 Season slates — confirmed and candidate shows
    ├── peers.json                   Peer venue metadata (5 peer types — see HOW_IT_WORKS.md)
    ├── context.json                 Weekly weather + economic context
    ├── awards.json                  Award-body records (built by scripts/build_awards.py)
    ├── factsheets.json              Per-show performance-schedule/price-level facts
    ├── media_signals.json           Media/press signal tracking
    ├── media_sources.json           Media source metadata
    ├── title_aliases.json           Show title alias/normalization map
    ├── venues.json                  Venue metadata for the box office model
    ├── versions.json                Page version numbers (see CLAUDE.md → Versioning)
...
**`js/pages/*.page.js` are not wired into any HTML page** — none of `dashboard.html`, `programming.html`, or `exec_summary.html` has a `<script src="js/pages/...">` tag; all three pages' actual logic lives in their inline `<script>` blocks instead. This looks like a stalled module-split refactor rather than dead weight left on purpose — worth a decision on whether to finish wiring these in or remove them, rather than treating the file tree above as a description of what's actually running.
...
├── run_pipeline.py             Manual entry point — chains process_touring.py,
